...
6.  「**フォーム**」 タブを選択し、「**メイン**」 フォーム タイプをクリックして開きます。 
    ...
    -   規定ではフォームには、「名前」 (プライマリ フィールド) と 「所有者」の 2 つのフィールドがあります。
...
9.  コード フィールドで選択したまま、 プロパティパネルの**読み取り専用フィールド**のチェックボックスをオンにします。
...
9.  ここで、ビューをクローンして、今日の訪問のための新しいビューを作成します。

10.  「**保存**」 ボタンの横にある下向き矢印を押して (ボタン自体を押さないように注意してください)、「**名前を付けて保存**」 を選択します。
...
15.  **「状態」** 行の 「**...**」 を クリックし、**「削除」** をクリックします。    

16.  このビューは、スケジュールされた開始日が今日のレコードのみを表示するようにフィルター処理されます。「**OK**」 をクリックして、条件を保存します。

15.  「**実際の開始**」 フィールドと 「**実際の終了**」 フィールドをビューに追加します。 

        > **注記:** ビューのステータスをフィルター処理しないため、完了した訪問を含む今日の訪問をすべて取得します。これらのフィールドは、完了した訪問と進行中の訪問を区別するのに役立ちます。
...
13.  **「検証」** をクリックして、アプリケーションで行われた変更を検証します。 
