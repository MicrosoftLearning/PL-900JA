...
9. 「*メール*」 を検索し、「**メール**」 コネクタを選択し、「**メール通知を送信する**」 アクションを選択する 
...
![Power Automate 訪問者の通知フロー](media/4-power-automate-notification.png)
...
4.  コマンド バーで、「**テスト**」 をクリックします。「**手動**」、「**保存してテスト**」 の順に選択します。
...
5. 「**新しい手順**」 をクリックします。「*最新*」 を検索して、「**Common Data Service (現在の環境)**」 を選択します。「**行の一覧表示**」 アクションを選択します。
...
2. まだ存在しない場合は、**セキュリティ スイープ** フローに移動します。

3. フローが開いたら、「**テスト**」 をクリックします。

4. 「**手動**」 を選択します。

5. 「**テストしてテスト**」 および 「**フローの実行**」 をクリックします。

6. フローが競合する場合は、「**完了**」 をクリックします。 

7. 「**それぞれに適用**」 を展開し、「**電子メールによる通知の送信**」 の手順を展開します。「**件名**」、「**電子メール本文**」 の値を確認してください。

8. セキュリティ スイープ フローの詳細への戻る矢印を選択します。コマンド バーで 「**オフにする**」 を選択します。これは、テスト システム上でフローがスケジュールどおりに実行されないようにするためです。
