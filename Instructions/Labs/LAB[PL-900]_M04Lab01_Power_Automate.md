...
**目標:** この演習では要件を実装する Power Automate フローを作成します。訪問者には、訪問に割り当てられた一意のコードを含む電子メールが送信されます。
...
9. 「*メール*」 を検索し、「**メール**」 コネクタを選択 し、「**メール通知を送信する**」 アクションを選択する 
   ...
   * 「**送信先**」 フィールドを選択し、動的コンテンツ リストから 「**電子メール**」 を選択します。「**訪問者の取得**」 の灰色のヘッダーの下にあることに注意してください。これは、前の手順で検索した訪問者に関連する電子メールを選択していることを意味します。 
...
14.  コマンド バーの 「**オフにする**」 をクリックして、フローをオフにします。

# 演習 #2: セキュリティ スイープ フローの作成

**オブジェクト:** この演習では要件を実装する Power Automate フローを作成します。セキュリティ スイープは 15 分ごとに実行され、訪問者のいずれかが予定時間を超過した場合、セキュリティが通知されます。
...
11.  「**メール**」 接続から 「**電子メールによる通知の送信**」 アクションを追加し、**各ループに適用**内にとどまります。

12.  「**宛先**」 に電子メール アドレスを入力します

13.  「**件名**」 フィールドに次のように入力します。「**訪問者 (値)**」 は、**訪問者の取得**手順の動的コンテンツです。
