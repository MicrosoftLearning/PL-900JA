...
8. **bc_Building** テーブルから  **bc_Buildingid** 列 をドラッグして、  **bc_Visit** テーブルの **bc_building** 列にドロップします。これにより、Power BI が関連データの表示に使用できる 2 つのエンティティ間のリレーションシップが作成されます。
...
4. 「**既存のダッシュボード**」 を選択し、 「**姓*キャンパス管理**」 ダッシュボードを選択し、「**ピン留めする**」 を押します。
