...
1.  ソリューションを作成する
    ...
    -   「**表示名**」 に、「**「あなたの姓」 キャンパス管理**」と入力します。

2.  発行元を作成する

    -   「**公開元**」 ドロップダウンをクリックし、「**公開元**」 を選択します。

    -   表示されるウィンドウで、「**表示名**」 に「**Bellows College**」、**プレフィックス**に「**bc**」を入力します。
...
2.  建物エンティティの作成
    ...
    -   「**完了**」 をクリックします。これにより、他のエンティティとフィールドの追加を開始する間に、バックグラウンドでエンティティのプロビジョニングが開始されます。
...
3. 「**状態**」 が**オフ** の場合は、「**データのインポート**」 の横にある 「**...**」 を 選択してから、「**オンにする**」 を選択します。

   > **重要:** エラー メッセージが表示された場合は、作成したエンティティとフィールドが上記の手順と一致していることを確認します。

4. 「**データのインポート**」 コンポーネントを選択します。Power Automate の新しいタブが開きます。ポップアップが表示された場合は、「**開始**」 をクリックします。 

5. 「**実行**」 をクリックしてから、プロンプトが表示されたら 「**フローの実行**」 をクリックします。

6. 「**完了**」 をクリックします。

7. フロー インスタンスが実行を完了するまで待機します。**28 日間の実行履歴**テーブルを更新して、フローがいつ実行されたかを確認できます。このフローの目的は、今後のラボのサンプル データを生成することでした。次のタスクでは、データのインポートが正常に完了したことを確認します。 
...
4. 「**建物**」 列の任意の値をクリックし、建物フォームが別のウィンドウで開くことを確認します。このウィンドウを閉じます。

5. 「**訪問者**」 列の任意の値をクリックし (ビューを右にスクロールする必要がある場合があります)、連絡先フォームが別のウィンドウで開くことを確認します。このウィンドウを閉じます。
