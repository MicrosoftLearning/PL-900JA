...
* ソリューションと発行元を作成します
* アプリケーション要件を満たすために必要な新規コンポーネントと既存のコンポーネントの両方を追加します。メタデータの説明 (エンティティおよびリレーションシップ) については、[データ モデル ドキュメント](https://github.com/MicrosoftLearning/PL-900-Microsoft-Power-Platform-Fundamentals/raw/master/Allfiles/Labs/Campus%20Management.png)を参照してください。Ctrl キーを押しながらクリックするか、リンクを右クリックして、データ モデル ドキュメントを新しいウィンドウで開くことができます。
...
2.  「**環境**」 を選択し、上部のメニューから 「**新規** 」をクリックします。これにより開きます
    メニューがウィンドウの右側に開きます。
...
1.  ソリューションを作成する
    ...
    -   画面の右上隅にある 「**環境**」 をクリックし、ドロップダウン メニューから環境を選択して、環境を選択します。 
...
2.  発行元を作成する
    ...
    -   ポップアップ ウィンドウで、**表示名**に「**Bellows College**」、**プレフィックス**に「**bc**」と入力します。
...
10. **1 つのビュー** と **1 つのフォーム**が選択されている必要があります。「**追加**」 をクリックします。
    これにより、選択したビューとフォームを持つ Contact エンティティが、新しく作成されたソリューションに追加されます。 
...
1.  キャンパス管理ソリューションを開いたまま、ブラウザーを使用する必要があります。表示されていない場合は、次の手順でキャンパス管理ソリューションを開きます。
...
1.  **キャンパス管理** ソリューションの「**訪問**」エンティティが引き続き表示されていることを確認します。表示されていない場合は、そのエンティティに移動します。
...
5.  トップ メニューから 「**ソリューション**」 を選択し、「**すべてのカスタマイズを発行する**」 をクリックします。
...
1. [CampusDataMap.xml](../../Allfiles/Labs/CampusDataMap.xml) をダウンロードします (まだダウンロードしていない場合)。
2. Power Platform 管理センター (https://admin.powerplatform.microsoft.com) に移動してサインインします。
...
7. Thew Campus Data Map ファイルが正常にインポートされる必要があります。エラーが発生した場合は、前に戻って、前のタスクで必要なエンティティとフィールドをすべて作成したことを確認してください。
8. トップ メニューから 「**ソリューション**」 を選択し、「**すべてのカスタマイズを発行する**」 をクリックします。
...
3. 「Bellows College」 環境を選択します。
4. 上部の 「**設定**」 をクリックします。
5. 「**データ管理**」 セクションを展開し、「**データ インポート ウイザード**」 を選択します。
6. **「データのインポート」** を押します。
...
11. **「完了」** を押します。
12. ここで、データのインポートが開始されます。「自分のインポート」 画面の右側にある 「更新」 ボタンを使用して、4 つのインポートのすべての 「**状態の理由**」 が 「**完了**」 になるまでテーブルを更新できます。これには数分かかる場合があります。

## タスク #3: データ インポートを確認します

1. **「キャンパス管理」** ソリューションを選択します。まだ開いていない場合は、make.powerapps.com に移動し、左側のウィンドウでソリューションをクリックしてソリューションを検索します。*
