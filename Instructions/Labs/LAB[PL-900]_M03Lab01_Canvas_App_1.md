...
3.  **Common Data Service** 接続を選択し、「**作成**」 をクリックします。
    ...
    -   「**電源アプリ スタジオへようこそ**」 ウィンドウが表示される場合があります。「**スキップ**」 をクリックします。
...
3.  プロパティ パネルから 「**TemplateSize**」 プロパティを選択します
...
1. 「**BrowseScreen1**」 を選択して、「BrowseGallery1」 の選択が解除されていることを確認します
...
8. 式で、`[@Visits]`を見つけて、`Filter(Visits,'Scheduled End' >= DatePicker1.SelectedDate)`に置き換えます。完全な式は次のようになります。
